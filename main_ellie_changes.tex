...
\begin{abstract}
Reducing health inequalities requires improved understanding of the reasons for local-level variation. Differences in local population characteristics and health system performance can both contribute. Identifying low- and high-performing local areas can help investigate what determines variations in performance.
Multilevel regression with post-stratification (MRP) was used to synthesise multiple data sources, using chlamydia testing as an example. National-level survey data can identify individual-level characteristics associated with chlamydia testing. This was combined with local-level demographic data to calculate the expected levels of testing in each local authority (LA) in England, allowing LAs to be identified where observed chlamydia testing rates were lower or higher than expected, given population characteristics.
